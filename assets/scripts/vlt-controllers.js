...
(function ($) {
	...
	function openMenu() {
		var menu = $('.popupMenu'),
			siteOverlay = $('.vlt-site-overlay'),
			popupContent = menu.find('.vlt-animated-block'),
			popupImg = menu.find('.vlt-slide-photo__inside img');
	...
	$(document).ready(function () {
		var menuOpen = $('.location-pin-img'),
			menuClose = $('#popup-Close'),
			siteOverlay = $('.vlt-site-overlay');
		...
		$(document).on('click', function (e) {
			if (menuIsOpen && !$(e.target).closest('.popupMenu').length && !$(e.target).closest('.location-pin-img').length) {
				e.preventDefault();
				closeMenu();
			}
		});
...
/* Background vedio play and pouse */

document.addEventListener("DOMContentLoaded", function() {
    const experienceSection = document.querySelector('[data-anchor="Experience"]');
    const video = document.getElementById("experience-background");
...
/***********************************************
 * WIDGET: LOGOTIPS CARUSEL
 ***********************************************/

document.addEventListener('DOMContentLoaded', function () {
    const track = document.querySelector('.carusel-track');
    const items = Array.from(document.querySelectorAll('.carusel-item'));
    const itemCount = items.length;
    let currentIndex = 0;
    let direction = 1; // 1 - oldinga, -1 - orqaga
